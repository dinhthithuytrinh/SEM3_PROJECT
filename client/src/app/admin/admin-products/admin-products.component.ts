...
export class AdminProductsComponent implements OnInit {
  ...
  constructor(
    ...
    this.productForm = this.fb.group({
      // Initialize originForm using FormBuilder
      id: [''],
  ...
  ngOnInit(): void {
    ...
    this.sharedService.currentSearchTerm.subscribe((term: string) => {
      // Đặt kiểu dữ liệu của term là string
  ...
  editClick(item: any) {
    if (item && item.id !== '0') {
      ...
      forkJoin([this.fetchTypes(), this.fetchOrigins()]).subscribe(() => {
        this.selectedProduct.productBrandId =
          this.selectedProduct.productBrandId || ''; // Gán giá trị mặc định nếu không có giá trị
        this.selectedProduct.productBrandId =
          this.selectedProduct.productBrandId || ''; // Gán giá trị mặc định nếu không có giá trị
        this.ActivateAddEditProductComponent = true;
      });
  ...
  sortResult(prop: any, asc: any) {
    this.ProductsList = this.ProductsListWithoutFilter.sort(function (
      a: any,
      b: any
    ) {
      if (asc) {
  ...
  applyFilter() {
    if (this.searchTerm.trim().length > 0) {
      // Sử dụng trim() để loại bỏ khoảng trắng thừa và kiểm tra độ dài của từ khóa tìm kiếm
